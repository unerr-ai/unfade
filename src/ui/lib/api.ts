...
export const api = {
  ...
  health: () => get<SystemHealth>("/api/system/health"),
  ...
  repos: {
    ...
    events: (id: string, params?: { from?: string; to?: string; limit?: number }) => {
      ...
      return get<{ events: CapturedEvent[] }>(`/api/repos/${id}/events?${qs}`);
    },
  },

  projects: {
    list: () => get<{ projects: Project[] }>("/api/projects"),
    discover: () => get<{ projects: DiscoveredProject[] }>("/api/projects/discover"),
    add: (path: string) => post<Project>("/api/projects", { path }),
    pause: (id: string) => post<void>(`/api/projects/${id}/pause`),
    resume: (id: string) => post<void>(`/api/projects/${id}/resume`),
    restart: (id: string) => post<void>(`/api/projects/${id}/restart`),
  },
  ...
  distill: {
    latest: () => get<DistillResponse>("/unfade/distill/latest"),
    byDate: (date: string) => get<DistillResponse>(`/unfade/distill/${date}`),
    generate: (date: string) => post<unknown>("/unfade/distill", { date }),
  },

  profile: {
    get: () => get<ProfileResponse>("/unfade/profile"),
  },

  setup: {
    discoverProjects: () =>
      get<{ projects: DiscoveredProject[]; cwd: string }>("/api/setup/discover-projects"),
    addProject: (path: string) => post<unknown>("/api/setup/add-project", { path }),
    ...
    progress: () => get<{ progress: number; status: string }>("/api/setup/progress"),
  },
  ...
  settings: {
    status: () =>
      ...
      }>("/unfade/settings/status"),
    saveLlm: (config: { provider: string; model: string; apiKey?: string; apiBase?: string }) =>
      post<unknown>("/unfade/settings/llm", config),
  },

  integrations: {
    status: () =>
      get<{ integrations: Array<{ name: string; status: string; connected: boolean }> }>(
        "/api/integrations/status",
      ),
    install: (name: string) => post<unknown>("/api/integrations/install", { name }),
  },
  ...
  logs: {
    list: (params?: { level?: string; component?: string; limit?: number }) => {
      ...
      if (params?.limit) qs.set("limit", String(params.limit));
      return get<{ logs: LogEntry[]; total: number }>(`/api/logs?${qs}`);
    },
  },

  cards: {
    generate: (params?: { date?: string; style?: string }) =>
      post<{ path: string }>("/unfade/cards/generate", params),
    list: () =>
      get<{ cards: Array<{ date: string; size: number; createdAt: string }> }>(
        "/unfade/cards/list",
      ),
    imageUrl: (date: string) => `/unfade/cards/image/${date}`,
  },
} as const;
